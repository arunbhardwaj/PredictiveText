...
public class MarkovWordTwo implements IMarkovModel {
	...
	private void buildHash(String key1, String key2) {
		...
		for (int index = indexOf(myText, key1, key2, 0); index < myText.length - 2; index = indexOf(myText, key1, key2, index + 1)) {
			if (index == -1) {
				break;
			}
	...
	private int indexOf(String[] words, String target1, String target2, int start) {
		...
		for (int i = start; i < words.length - 1; i++) {
			if ( target1.equalsIgnoreCase(words[i]) && target2.equalsIgnoreCase( words[i+1] ) ) {
				return i;
			} 
		}
		return -1;
...
//	}
	...
	public void testIndexOf() {
		...
		System.out.println(indexOf(words, "frog", "and",  5));
